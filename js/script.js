document.addEventListener("DOMContentLoaded", function () {
  ...
  if (window.matchMedia("(pointer: coarse)").matches) {
    document
      .getElementById("header__menu-link--notifications")
  ...
  window.addEventListener("resize", function () {
    if (window.matchMedia("(pointer: coarse)").matches) {
      document
        .querySelector("#header__menu-link--notifications")
        .addEventListener("click", function (e) {
          if (
            e.target === this ||
            e.target === this.querySelector(".header-notification-unread") ||
  ...
  document.getElementById(
    "header__menu-link--language"
  ).style = `background: url('./img/icons/languages/${user_lang}.svg') center / contain no-repeat;`;
  ...
  if (barcharts.length > 0) {
    barcharts.forEach((barchart) => {
      ...
      let nav_radio_labels = barchart.parentElement.querySelectorAll(
        ".barchart-nav-radio-label"
      );
